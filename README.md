...
#### Bonus: Batch Download a Season

[`download_season.py`](download_season.py) downloads a whole season through the `/download/.../file` endpoint at a chosen quality — with resume-safe skipping, retries, and per-episode progress:

```bash
python download_season.py --search "naruto shippuden" --quality 480p
python download_season.py --id 1735 --start 1 --end 24 --quality 720p -o naruto-s01
python download_season.py --id 1735 --dry-run
```

Files land as `NARUTO Shippuuden - E001 - 480p.mp4`. Full reference in [`APIUSAGE.txt`](APIUSAGE.txt).
